...
export default function Orb({ hue = 0, hoverIntensity = 0.2, rotateOnHover = true, forceHoverState = false }) {
  ...
  const frag = /* glsl */ `
    ...
    vec4 mainImage(vec2 fragCoord) {
      ...
      /* 🛡️ RESTORED: Removed the 0.75 scale to make it round and large again */
      ...
      /* 🛡️ DYNAMIC: Distortion only applies when hovering */
  ...
  useEffect(() => {
    ...
    let lastTime = 0;
    let currentRot = 0;
    const rotationSpeed = 0.5; // Base slow rotation
    ...
    const update = t => {
      ...
      /* 🛡️ ROTATION FIX: Always rotates, maybe speed up slightly on hover */
      currentRot += dt * (rotationSpeed + (effectiveHover * 0.4));
    ...
    return () => {
      ...
      if (container && gl.canvas && container.contains(gl.canvas)) { container.removeChild(gl.canvas); }
